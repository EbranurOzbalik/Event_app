...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _tryRecoverFromKeychainError() async {
    try {
      await FirebaseAuth.instance.signOut();
    } catch (_) {
      // Keychain bozuk durumda olabilir; signOut başarısız olsa da retry devam eder.
    }
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        final isKeychainError =
            ...
            e.message?.toLowerCase().contains('keychain') == true;

        if (isKeychainError) {
          await _tryRecoverFromKeychainError();
        }

        if ((!isNetworkError && !isKeychainError) || attempt == maxAttempts) {
          rethrow;
        }
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      switch (e.code) {
        ...
        case 'keychain-error':
          msg =
              'iOS keychain hatası. Simülatörü sıfırlayıp uygulamayı yeniden kur ve tekrar dene.';
          break;
        default:
          msg =
